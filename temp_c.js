  ...
  },
  {
    ...
    // Lógica de importación manual
    document.getElementById('fileImportBD').addEventListener('change', function (e) {
      if (!e.target.files.length) return;
      const file = e.target.files[0];
      CreacionesDB.importJSON(file, function (success, count) {
        if (success) {
          creaciones = CreacionesDB.getAll();
          render();
          showToast(`Base de datos importada exitosamente (${count} recetas).`);
          setLive("Base de datos importada");
        } else {
          showToast("Error al importar la BD. El archivo no es válido.");
        }
      });
